...
║       Audio & Video Generation (Free Tier)                              ║
║                                                                           ║
║   Using:                                                                 ║
║   • Azure Text-to-Speech (Free: 500K chars/month)                       ║
║   • FFmpeg (Open Source)                                                ║
...
Write-Host "1️⃣  PHASE 1: Validating Prerequisites" -ForegroundColor Cyan
...
if ([string]::IsNullOrWhiteSpace($TtsKey)) {
    Write-Host "❌ Error: Azure TTS Key is required" -ForegroundColor Red
    Write-Host "   Use: ./Run-AudioVideo-Generation.ps1 -TtsKey 'your-key'" -ForegroundColor Yellow
    $prereqFailed = $true
}
else {
    Write-Host "✅ Azure TTS Key provided" -ForegroundColor Green
}
...
if (Test-Path $BaseFolder) {
    Write-Host "✅ Base folder found: $BaseFolder" -ForegroundColor Green
}
else {
    Write-Host "❌ Base folder not found: $BaseFolder" -ForegroundColor Red
    $prereqFailed = $true
}
...
if ($mdFiles.Count -gt 0) {
    ...
    Write-Host "❌ No markdown files found in: $BaseFolder" -ForegroundColor Red
    $prereqFailed = $true
}

if ($prereqFailed) {
    Write-Host "`n❌ Prerequisites not met. Please fix issues above and try again." -ForegroundColor Red
    exit 1
}

Write-Host "`n✅ All prerequisites validated`n" -ForegroundColor Green

# ============================================================================
# PHASE 2: TEST AZURE TTS CONNECTION
# ============================================================================

Write-Host "2️⃣  PHASE 2: Testing Azure TTS Connection" -ForegroundColor Cyan
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`n" -ForegroundColor Cyan
...
"@
    ...
    if ((Test-Path $testFile) -and (Get-Item $testFile).Length -gt 0) {
        Write-Host "✅ Azure TTS connection successful" -ForegroundColor Green
        Remove-Item -Path $testFile -Force -ErrorAction SilentlyContinue
    }
    else {
        Write-Host "❌ Azure TTS returned empty response" -ForegroundColor Red
        exit 1
    }
}
catch {
    Write-Host "❌ Azure TTS connection failed: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host "   Check your TTS key and region" -ForegroundColor Yellow
    exit 1
}

Write-Host "`n✅ Connection test passed`n" -ForegroundColor Green

# ============================================================================
# PHASE 3: GENERATE AUDIO
# ============================================================================

Write-Host "3️⃣  PHASE 3: Generating Audio Files (MP3)" -ForegroundColor Cyan
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`n" -ForegroundColor Cyan

$generateAudioScript = Join-Path $BaseFolder "Generate-Audio.ps1"

if (!(Test-Path $generateAudioScript)) {
    Write-Host "❌ Generate-Audio.ps1 not found at: $generateAudioScript" -ForegroundColor Red
...
try {
    ...
    Write-Host "`n❌ Error during audio generation: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
...
if ($audioFiles.Count -eq 0) {
    Write-Host "❌ No audio files were generated" -ForegroundColor Red
    exit 1
}

Write-Host "`n✅ Audio generation complete ($($audioFiles.Count) files)`n" -ForegroundColor Green

# ============================================================================
# PHASE 4: GENERATE VIDEOS
# ============================================================================

Write-Host "4️⃣  PHASE 4: Generating Video Files (MP4)" -ForegroundColor Cyan
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`n" -ForegroundColor Cyan

$generateVideosScript = Join-Path $BaseFolder "Generate-Videos.ps1"

if (!(Test-Path $generateVideosScript)) {
    Write-Host "⚠️  Generate-Videos.ps1 not found. Skipping video generation." -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host "⚠️  Error during video generation: $($_.Exception.Message)" -ForegroundColor Yellow
...
if ($videoFiles.Count -gt 0) {
    Write-Host "✅ Video generation complete ($($videoFiles.Count) files)`n" -ForegroundColor Green
}
else {
    Write-Host "⚠️  No video files were generated. This may require ImageMagick.`n" -ForegroundColor Yellow
}
...
Write-Host "5️⃣  PHASE 5: Summary & Next Steps" -ForegroundColor Cyan
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`n" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "  1. ✅ Share audio files (podcast format)" -ForegroundColor White
Write-Host "     → Host on Spotify, Apple Podcasts, or internal server" -ForegroundColor Gray
Write-Host ""

if ($videoFiles.Count -gt 0) {
    Write-Host "  2. ✅ Share video files" -ForegroundColor White
    Write-Host "     → Upload to YouTube, Azure Media Services, or LMS" -ForegroundColor Gray
}
else {
    Write-Host "  2. ⏭️  Generate videos (optional)" -ForegroundColor White
...
Write-Host ""
Write-Host "  3. 📤 Upload to Azure (free tier)" -ForegroundColor White
...
Write-Host "  & 'path\to\file.mp3'" -ForegroundColor Gray
Write-Host ""
Write-Host "  # Calculate total size" -ForegroundColor Yellow
Write-Host "  (Get-ChildItem '$audioFolder' -Filter '*.mp3' | Measure-Object -Property Length -Sum).Sum / 1GB" -ForegroundColor Gray
Write-Host ""
